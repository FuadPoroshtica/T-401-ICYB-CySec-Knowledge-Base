...
- HTML, CSS, ... → DOM (Document Object Model)
    - The DOM represents the structure of a web page and can be manipulated by scripts, which can introduce security risks such as [Cross-Site Scripting (XSS)](../Terminology/Attacks/XSS.md) if not properly handled.
    - Understanding the DOM is crucial for identifying potential vulnerabilities in web applications.
...
- Agency Surface
  - Plugins, API calls, Code Execution. If the AI has access to external plugins, APIs, or can execute code, attackers can exploit these capabilities to perform malicious actions.
...
- *Mitigation:*
    ...
    - **URL analysis tools.** Using tools that analyze URLs for suspicious patterns or characteristics can help identify phishing sites.
