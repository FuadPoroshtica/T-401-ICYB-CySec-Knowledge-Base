...
```
File globs vs reges:
**Globs:** Shell interpret for filenames (star.txt,? = any single char)
**Regex:** Tools interpret for text content (.= any char, star = zero or more)
Regex cheatsheet: . any char, ^ start of line, $ end of line, star zero or more, [a-z] char range, (X|Y) X or Y
#### Loops & functions
bash loop:
```bash
for f in *.txt; do
...
```
dont do this, download it first and inspect it and then maybe run it
**History File Leaks**
```bash
mysql -pSecret123
```
...
**Path hijacking**
dont add . (current dir) to $Path, attack can create fake ls commands that run malware
**Permission Laziness**
```bash
chmod 777 file
```
