Day 1- intro
1-How the virus / attack worked
2- What was the impact
3- Which weakness was used

Creeper 1971: 
1. Creeper was a worm — a type of malwere that replicates itself and spreads to other systems. In this case, its targets were Digital Equipment Corporation (DEC) computers which were linked to ARPANET.
2. connected teletype computer screens displayed the phrase: “I’m the creeper, catch me if you can!”
3. Unprotected FTP access, Lack of security measures, open network architecture, no concept of malware protection. Its creator had simply wanted to create an experimental, self-duplicating program to illustrate that it was possible.
...
3. A hole in the debug mode of the Unix sendmail program, a buffer overflow or overrun hole in the finger network service, the transitive trust enabled by people setting up network logins with no password requirements via remote execution and exploited weak passwords
...
(WannaCry is an example of crypto ransomware, a type of malware used to extort money. Ransomware does this by either encrypting valuable files(crypto ransomware), so you are unable to read them, or by locking you out of your computer(locker ransomware), so you are not able to use it.)
...
3. It uses the EternalBlue exploit to spread between connected systems that where not patched against the vulnerability.
https://www.fortinet.com/resources/cyberglossary/wannacry-ransomware-attack
(**EternalBlue**, a zero-day vulnerability in devices that use an old version of SMB. It exploits a vulnerability in the Microsoft implementation of the Server Message Block (SMB) Protocol. This dupes a Windows machine that has not been patched against the vulnerability into allowing illegitimate data packets into the legitimate network. These data packets can contain malware such as a trojan, ransomware or similar dangerous program.)

Day 2- Virtualization

1. Under what specific scenarios would you have to choose VMs over containers for security reasons, and vice-versa? 
VM are better if security is the priority since each VM operates as a fully isolated computer with its own OS and applications, so if one VM is compromised, it’s difficult for the attack to spread to other VMs or the host machine. But containers share the host operating system kernel, so a vulnerability in the kernel can potentially affect all containers. So VMs are better suited for workloads that handle sensitive data or require strict security and compliance. Containers might be chosen when performance is the priority as containers are faster, lighter, and more resource efficient, the fast startup time of containers directly contributes to scalability, making them ideal for microservices architectures.
https://monovm.com/blog/vm-vs-containers/

2. Research historical examples of VM escape vulnerabilities (e.g., specific CVEs for VMware, VirtualBox, KVM, or Docker). What were the root causes?
(VM escape is the process of a program breaking out of the VM on which it is running and interacting with the host operating system.)
  * CVE-2025-22224 is a Time-of-Check Time-of-Use (TOCTOU-is a class of software bugs caused by a race condition involving the checking of the state of a part of a system (such as a security credential) and the use of the results of that check.) vulnerability in VMware ESXi and Workstation, rated critical with a CVSS score of 9.3. It stems from a heap overflow in the Virtual Machine Communication Interface (VMCI), enabling an out-of-bounds write. An attacker with local admin privileges on a VM can exploit this to execute code as the VMX process on the host.
...
3. Side-channel attacks:  
	1. What is a side-channel attack (in the context of virtualization)?  
...
4. What are the risks associated with using untrusted or vulnerable container images from public repositories? 
