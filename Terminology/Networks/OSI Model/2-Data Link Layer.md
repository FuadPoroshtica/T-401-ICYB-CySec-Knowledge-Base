...
The Data Link Layer is divided into two sublayers: the Media Access Control (MAC) layer, which controls access to the physical medium, and the Logical Link Control (LLC) layer, which manages communication between devices on the same network.
