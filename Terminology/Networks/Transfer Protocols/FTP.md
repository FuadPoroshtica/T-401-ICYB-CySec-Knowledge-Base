...
FTP does not provide built-in encryption, which means that data, including usernames and passwords, is transmitted in plain text. To enhance security, secure variants of FTP, such as FTPS (FTP over <abbr title="Secure Sockets Layer">SSL</abbr>/[TLS](<../TLS.md>)) and SFTP (SSH File Transfer Protocol), are commonly used.
